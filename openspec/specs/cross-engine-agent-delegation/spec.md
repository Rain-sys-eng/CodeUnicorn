# cross-engine-agent-delegation Specification

## Purpose

定义 CodeUnicorn 跨引擎 Agent-to-Agent delegation 的 control-plane 契约：immutable run/lineage、可信 source identity、现有 runtime/event/approval ownership、durable recovery、continuation/cancel/context/worktree、Parallel/DAG 与 collaboration projection。
