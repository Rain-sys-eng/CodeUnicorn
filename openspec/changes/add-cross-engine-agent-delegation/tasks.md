...
- [x] 11.1 Rust focused tests + rustfmt check（仅改动文件）。**GitHub CI run 15 真实完成 Rust suite（2647 passed / 15 existing-baseline failed），Agent Bridge/Qoder/DAG focused tests 全部通过；run 20 对声明的 27 个 changed files 执行 non-recursive `rustfmt --edition 2021 --check` 并通过，且 `memory-kind-contract` 为 success。daemon fail-closed stubs、graph backend `Sync` boundary 与最终格式化结果均有真实 Rust 编译证据。**
...
- [x] 11.3 MCP contract tests、parallel/cancel/recovery integration tests、fake-engine tests。**managed Claude/Codex/Qoder transport contract coverage 已执行：authenticated `tools/list` 精确枚举七个 Bridge tools、legacy route fail closed、unknown runtime locator、bearer rejection、unknown-tool JSON-RPC error，以及 process/ACP transport 注入不覆盖用户配置。Gateway 窄 `AgentBridgeMcpBackend` seam 的 deterministic fake runtime 覆盖七工具 delegate→status→wait/result→send→cancel、Approval accept/reject、continuation identity reuse、nested exact runtime parent、cross-source/workspace fail closed 与 stale-runtime recovery。Graph coordinator 的 `AgentGraphBridgeBackend` fake Bridge integration 并发 tick root→双 fan-out→join，断言每个 immutable run 只 create/dispatch 一次；CI run 15 中新增 graph/Qoder/source tests 均为 `ok`。**
- [ ] 11.4 手工验收 Claude→Codex、Codex→Claude、并行三 Agent、nested delegation、worktree isolation。
- [x] 11.5 strict OpenSpec validate / consistency sync。**`OPENSPEC_TELEMETRY=0 npm exec --offline --yes @fission-ai/openspec@latest -- validate add-cross-engine-agent-delegation --strict --no-interactive` 已通过；delta requirements 已逐字同步到 main `cross-engine-agent-delegation` capability（仅增加 title/Purpose/Requirements wrapper），main spec index/count 同步为 541。当前 executor 再次离线调用 CLI 被连接策略阻断，不把该环境限制伪报为 validate failure。**
- [ ] 11.6 更新 foundation ADR，写 verification，sync main specs，archive change。**foundation ADR、`verification.md` 与 main spec sync 已完成；archive 必须等待 11.4 真实 runtime 手工证据，避免把未验收 change 提前归档。**
