...
When the effective Codex configuration contains `model_providers.<provider>.env_key`, the desktop app SHALL resolve a missing value from the user's allowlisted login/interactive shell before spawning the Codex app-server, and SHALL resolve all missing keys in a single bounded shell invocation.
