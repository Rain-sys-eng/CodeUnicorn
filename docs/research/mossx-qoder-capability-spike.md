...
> 日期：2026-08-21
> CLI：`qodercli` `1.1.27`（global bin `/Users/chenxiangning/.local/bin/qodercli` → `~/.qoder/bin/qodercli/qodercli-1.1.27`）
...
> 登录态：`qodercli status -o json` → `logged_in: true`（browser login）；**本机模型 API 当前不可达**（两个模型均 `Network attempt failed at unknown`，~250–530ms 内失败），成功 turn 的流式事件面未能 live 采集，相关条目按纪律标 `unknown`
...
| 未 live 观测 | `agent_thought_chunk`、`tool_call`、`tool_call_update`、`plan`：binary strings + ACP v1 标准词汇 + Kimi ACP spike 同协议实测，**Qoder live 未验证**（上游网络阻断） | strings + S3 spike 交叉证据 |
...
| 中途输入 | `_meta.qoder.promptQueueing: true`（声明支持排队），未 live 验证 | initialize |
| 取消 | `session/cancel` notification（binary + ACP 标准），未 live 验证 | strings |
...
| Tool Progress | `tool_call` / `tool_call_update` | 真实（未 live，binary+协议交叉证据） |
| Completion | prompt response（成功路径 `stopReason`；错误路径 JSON-RPC error） | **真实**（错误路径已实测） |
| Interrupt | `session/cancel` → `stopReason:"cancelled"`（ACP 标准） | 未 live 验证 |
...
- ACP usage 面未观测（无成功 turn）；binary 含 usage 字符串。**标 unknown，成功 turn 后补测。**
...
| streaming.reasoning | unknown | `agent_thought_chunk` binary+协议交叉证据，live 未验证 |
| streaming.tool-output | unknown | `tool_call/tool_call_update` 同上，live 未验证 |
...
| input.mid-turn | unknown | `_meta.qoder.promptQueueing:true` 声明，live 未验证 |
| session.resume | supported | `session/resume` / `session/load` 实测 |
| session.fork | supported | `sessionCapabilities.fork` 声明（initialize 实测） |
...
- **R1（高）成功 turn 未采集**：streaming.reasoning / tool-output / usage / cancel / fork / promptQueueing 全部依赖协议交叉证据；有可用账号后必须补黄金 turn 并把 unknown 项升级为实测值。
- **R2（中）隐藏 flag**：`--acp` / `--yolo` 不在 `--help`，版本升级可能变更；cache key 必须含 version+sha256+capabilities hash，变了重 probe。
- **R3（中）`session/list` 为空**：本机无成功 turn 时返回空；sidebar 历史首期依赖 ACP list + mossx 本地 transcript，不做磁盘文件扫描。
...
- probe6 golden turn 尝试（qmodel_38max，网络失败）：`probe6-golden-turn.transcript.ndjson`
