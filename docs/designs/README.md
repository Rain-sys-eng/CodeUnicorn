...
| [`pi-native-features/index.html`](pi-native-features/index.html) | Pi 接入设计稿（真壳 base：真实 CSS @import + 运行时 DOM dump）：**★ `final-recommended.html` 最终推荐态**（2 全屏树 × 3 tab 分支 chip，pi native 独立增强）/ 4 共同分叉（mossx 级 fork UX）/ 1 侧栏树 / 2 沉浸树 / 3 内嵌分叉；跨引擎调研见 `docs/research/session-fork-tree-cross-engine-capability.md`；幕布不动 | 浏览器打开 gallery |
