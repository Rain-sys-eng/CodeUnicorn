...
/// before any Bridge mutation is allowed. The legacy workspace-only MCP route is intentionally
/// rejected for Agent Bridge tools because it cannot distinguish provider-scoped runtimes.
...
mod tests {
    ...
    #[tokio::test]
    async fn live_runtime_locator_mints_source_identity_and_native_session() {
        let manager = ClaudeSessionManager::new();
        ...
        assert_eq!(source.engine_id, "claude");
        assert_eq!(
            source.logical_session_id.as_deref(),
            Some(session.runtime_locator())
        );
        assert_eq!(
            source.native_session_id.as_deref(),
